 ...
 */
  ...
  */
  parseCsvRows(template: string, csvRows: CsvRow[]) : ParsedCsv {
    ...
    console.log(`\t\t\tdouble for loop: ${(end - mid)/ 1000}`);
    console.log(`\t\tparseCsvRows: ${(end-start)/ 1000}`);
  ...
  */
  async parseCsv(
    ...
    const mid = performance.now();
    console.log(`\t\tfileReader.readAsText: ${(mid - start) / 1000}`);
    ...
    return new Promise((res, rej) => {
      fileReader.addEventListener('loadend', () => {
        ...
        console.log(`\t\tcsvtojson: ${(jsonEnd - jsonStart) / 1000}`);
...
// TODO: filter out non statvarobs templates and make them mcfs
/*
generate datapoints:
- separate templates
- non-statvarobs become their own mcf strings
- statvarobs get added to template list
- retuned timeDataObject also has
*/
